...
export default function LogoIcon({ iconTheme }: IconProps) {
  return (
    <a aria-label="Home" href="/">
      {/* mobile logo icon */}
      <svg viewBox="0 0 32 32" aria-hidden="true" className="h-8 sm:hidden">
        <rect
          clip-path="url(#:rg:-clip)"
          className={`
          ${iconTheme === theme.dark ? "fill-white" : "fill-neutral-950"}
            h-8 transition-all duration-300  w-0 group-hover/logo:w-8`}
        ></rect>
        <use
          href="#:rg:-path"
          className={`${
            iconTheme === theme.dark ? "stroke-neutral-950" : "stroke-white"
          }`}
          fill="none"
          stroke-width="1.5"
        ></use>
        <defs>
          ...
          ></path>
          <clipPath id=":rg:-clip">
            <use href="#:rg:-path"></use>
          </clipPath>
        </defs>
      </svg>
      {/* desktop logo icon with logotype */}
      <svg
        viewBox="0 0 130 32"
        aria-hidden="true"
      ...
      >
        ...
        >
          <rect
            clip-path="url(#:rh:-clip)"
            className="h-8 transition-all duration-300 hover:fill-neutral-950 w-0 group-hover/logo:w-8"
          ></rect>
          <use
            href="#:rh:-path"
            className={`${
          ...
          <defs>
            ...
            ></path>
            <clipPath id=":rh:-clip">
              <use href="#:rh:-path"></use>
            </clipPath>
          </defs>
        </svg>
        {/* Below is the logotype */}
        <path
          className={`  ${
            iconTheme === theme.dark ? "fill-neutral-950" : "fill-white"
          }`}
